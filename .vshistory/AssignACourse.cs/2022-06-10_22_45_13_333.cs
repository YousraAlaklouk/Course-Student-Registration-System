    ...
    public partial class AssignACourse : Form
    {
        ...
        private void butFltr_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                connection.Open();
                SqlCommand cm = new SqlCommand("SELECT C.CourseID ,I.FirstName FROM Courses C INNER JOIN Assign A ON A.CourseID = C.CourseID INNER JOIN Instructors I ON I.InstructorNumber = A.InstructorNumber WHERE C.State = 'open' AND C.CourseID = " + combCrs.SelectedValue + "", connection);
