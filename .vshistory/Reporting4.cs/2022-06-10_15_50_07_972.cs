        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (radioStu.Checked)
            {
                ...
                try
                {
                    ...
                    connection.Open();
                    SqlCommand sqlCommand = new SqlCommand("SELECT COUNT(DATEDIFF(month, GETDATE(), '" + Convert.ToDateTime(txtStu.Text) + "') ) FROM Students", connection);
                    SqlDataAdapter sda = new SqlDataAdapter();
...

            {
                try
                {
                    ...
                    connection.Open();
                    SqlCommand sqlCommand = new SqlCommand("SELECT COUNT(DATEDIFF(month, GETDATE(), '" + Convert.ToDateTime(txtStu.Text) + "') ) FROM Instructors", connection);
                    SqlDataAdapter sda = new SqlDataAdapter();
                    sda.SelectCommand = sqlCommand;
                    Instructors = new DataTable();
                    sda.Fill(Instructors);
...

            {
                try
                {
                    ...
                    connection.Open();
                    SqlCommand sqlCommand = new SqlCommand("SELECT COUNT(DATEDIFF(month, GETDATE(), '" + Convert.ToDateTime(txtStu.Text) + "') ) FROM Courses", connection);
...

            {
                try
                {
                    ...
                    connection.Open();
                    SqlCommand sqlCommand = new SqlCommand("SELECT  COUNT(*) FROM Courses WHERE DATE = '" + Convert.ToDateTime(txtCrsDat.Text) + "'", connection);
                    SqlDataAdapter sda = new SqlDataAdapter();
                    sda.SelectCommand = sqlCommand;
                    Courses = new DataTable();
                    sda.Fill(Courses);
