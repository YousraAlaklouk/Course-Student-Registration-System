...
namespace Course_Student_Registration_System
{
    public partial class CourseCreation : Form
    {
        ...
        private void crtButt_Click(object sender, EventArgs e)
        {
            ...
            if (connection.State == ConnectionState.Open)
            {
                try
                {
                    ...
                    SqlCommand cr = new SqlCommand("SELECT CourseID FROM Courses WHERE Name = '@n' AND Title = '@t'", connection);
                    cr.Parameters.AddWithValue("@n", txtCrsNm.Text.Trim());
                    cr.Parameters.AddWithValue("@t", txtCrsTi.Text.Trim());
                    labCrsId.Text = cr.ExecuteScalar().ToString();
                }
                catch (Exception ex)
                {
        ...
        private void butAs_Click(object sender, EventArgs e)
        {
            try
            {
/*                int course = cr.ExecuteScalar();
*/                connection.Open();
                SqlCommand sd = new SqlCommand("INSERT INTO Assign VALUES (@crs,@ins1) ", connection);
                SqlCommand sr = new SqlCommand("INSERT INTO Assign VALUES (@crs,@ins2)", connection);
                SqlCommand ss = new SqlCommand("INSERT INTO Assign VALUES (@crs,@ins3)", connection);

                sd.Parameters.AddWithValue("@crs",labCrsId.Text);
                ...
                {
                    sr.Parameters.AddWithValue("@crs", labCrsId.Text);
                    sr.Parameters.AddWithValue("@ins2", combInstN2.SelectedValue);
                ...
                {
                    ss.Parameters.AddWithValue("@crs", labCrsId.Text);
                    ss.Parameters.AddWithValue("@ins3", combInstN3.SelectedValue);
