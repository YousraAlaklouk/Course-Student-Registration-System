        ...
        private void opnBut_Click(object sender, EventArgs e)
        {
            ...
            if (connection.State == ConnectionState.Open)
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE Courses SET State= @state ,DurationFrom= @from , DurationTO= @to, PricePerMonth= @price, CourseRoom= @room , Days =@day, Time= @time  WHERE CourseID=@ID", connection);
