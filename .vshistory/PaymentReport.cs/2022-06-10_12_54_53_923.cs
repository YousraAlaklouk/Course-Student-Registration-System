...
namespace Course_Student_Registration_System
{
    public partial class PaymentReport : Form
    {
        ...
        private void PaymentReport_Load(object sender, EventArgs e)
        {
            ...
            connection.Close();
            checkStdComp_CheckedChanged(sender, e);
            checkDntComp_CheckedChanged(sender, e);
        }
        ...
        private void butFiltr_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    SqlDataAdapter cmd = new SqlDataAdapter("SELECT  P.PaymentID , P.StudentNumber ,S.Name , P.Amount FROM Payment P INNER JOIN Students S ON S.StudentNumber = P.StudentNumberWHERE P.PaymentID =" + txtsrch.Text + " OR S.Name LIKE '%" + txtsrch.Text + "%'", connection);
                    Payment = new DataTable();
                    //to fill the data grid view according to the text written
                    cmd.Fill(Payment);
