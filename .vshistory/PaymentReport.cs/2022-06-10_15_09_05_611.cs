        ...
        private void butFiltr_Click(object sender, EventArgs e)
        {
            if (txtsrch.TextLength > 0)
            {
                ...
                try
                {
                    ...
                    {
                        SqlDataAdapter cmd = new SqlDataAdapter("SELECT  P.PaymentID , P.StudentNumber ,S.Name , P.Amount FROM Payment P INNER JOIN Students S ON S.StudentNumber = P.StudentNumber WHERE P.Amount = 0 OR S.Name LIKE '%" + txtsrch.Text + "%'", connection);
                        Payment = new DataTable();
                    ...
                    {
                        SqlDataAdapter cmd = new SqlDataAdapter("SELECT  P.PaymentID , P.StudentNumber ,S.Name , P.Amount FROM Payment P INNER JOIN Students S ON S.StudentNumber = P.StudentNumber WHERE P.Amount LIKE '-%' OR S.Name LIKE '%" + txtsrch.Text + "%'", connection);
                        Payment = new DataTable();
                        //to fill the data grid view according to the text written
                        cmd.Fill(Payment);
                    ...
                    else
                    {
                        ...
                        SqlDataAdapter cmd = new SqlDataAdapter("SELECT  P.PaymentID , P.StudentNumber ,S.Name , P.Amount FROM Payment P INNER JOIN Students S ON S.StudentNumber = P.StudentNumber WHERE P.PaymentID =" + txtsrch.Text + " OR S.Name LIKE '%" + txtsrch.Text + "%'", connection);
                        Payment = new DataTable();
                        //to fill the data grid view according to the text written
                        cmd.Fill(Payment);
