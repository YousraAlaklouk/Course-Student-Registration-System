        ...
        private void butFiltr_Click(object sender, EventArgs e)
        {
            if (txtsrch.TextLength > 0)
            {
                ...
                try
                {
                    ...
                    {
                        SqlDataAdapter cmd = new SqlDataAdapter("SELECT  P.PaymentID , P.StudentNumber ,S.Name , P.Amount FROM Payment P INNER JOIN Students S ON S.StudentNumber = P.StudentNumber WHERE P.Amount >= 0 AND (S.Name LIKE '%" + txtsrch.Text + "%'OR P.StudentNumber="+txtsrch.Text+" )", connection);
                        Payment = new DataTable();
