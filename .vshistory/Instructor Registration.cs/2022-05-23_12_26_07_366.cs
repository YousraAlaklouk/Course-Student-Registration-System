        ...
        private void subButt_Click(object sender, EventArgs e)
        {
            ...
            if (connection.State == ConnectionState.Open)
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO Instructors VALUES( @job ,@van ,@first,@last ,@mid , @id, @grad,@birth," +
                    "@dis ,@email,@phone,@address,@postcode,@degree,@bauni,@bacmajor,@masuni,@masspe,@phduni,@phdspe,@subjects)", connection);
                ...
                cmd.Parameters.AddWithValue("id", Convert.ToInt32(txtIDNm.Text));
                cmd.Parameters.AddWithValue("@grad", Convert.ToString(txtGrDate.Text));
                cmd.Parameters.AddWithValue("@birth",Convert.ToString(txtBD.Text));
                ...
                cmd.Parameters.AddWithValue("@postcode",Convert.ToInt32( txtPc.Text ));
                cmd.Parameters.AddWithValue("@bauni", txtBacUni.Text.Trim());
                cmd.Parameters.AddWithValue("@bacmajor", txtBacMaj.Text.Trim());
                cmd.Parameters.AddWithValue("@degree", comboDegree.SelectedText);
