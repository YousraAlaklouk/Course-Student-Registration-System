...
/*        public static string SetValueForText1 = "";
        ...
        {
            // INSERT to the instructors
            connection.Open();
            DataTable dtResult = new DataTable();
            if (connection.State == ConnectionState.Open)
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO Instructors VALUES( @job ,@van ,@first,@last ,@mid , @id, @grad,@birth," +
                    "@dis ,@email,@address,@phone,@postcode,@degree,@bauni,@bacmajor,@masuni,@masspe,@phduni,@phdspe,@subjects, GETDATE())", connection);
...
/*                SetValueForText1 = txtFn.Text;
                ...
                if (i != 0)
                {
                    ...
                    Instructor_Registration.fname = txtFn.Text.Trim();
                    Instructor_Registration.lname = txtLn.Text.Trim();
