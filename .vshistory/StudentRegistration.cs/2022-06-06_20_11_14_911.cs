...
*/
        ...
        {
            connection.Open();
            SqlCommand cmd = new SqlCommand("SELECT Department , DepartmentID FROM Department WHERE FaculityId = @ID ", connection);
            cmd.Parameters.AddWithValue("@ID", comboFacu.SelectedValue);

            SqlDataAdapter adapter2 = new SqlDataAdapter(cmd);
            adapter2.SelectCommand = cmd;
            DataTable Institute = new DataTable();
            adapter2.Fill(Institute);
            comboDep.DataSource = Institute;
